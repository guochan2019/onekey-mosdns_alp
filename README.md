...
# 方式二：GitHub 镜像加速（直连 GitHub 受限的环境）
...
> **GitHub 下载镜像 fallback（2026-09-09）**：脚本与每周 cron 的 update-mosdns.sh 均已内置 `dl_gh`——顺序 **gh-proxy.com → ghfast.top → 官方垫底**，直连 GitHub 受限的环境自动走镜像（官方 release 直连实测超时）。版本查询走 api.github.com 直连（gh-proxy 对其 403），失败用内置 FALLBACK 版本号。
